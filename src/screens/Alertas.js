...
export default class Alertas extends Component {
  ...
  getAlertsData = async () => {
    try {
      ...
      const msg = await axios.post(`${server}/alertas`, {trackers})
      const total = await axios.put(`${server}/alertas/count`, {trackers})
      this.setState({
        list: msg.data,
        fetchNewData: false,
        notificationCount: total.data[0].c - msg.data.length
      })
    } catch (err) {
      showError(err.response.data)
    }
  ...
  render() {
    return (
      <View style={styles.container}>
        <Header title='Alertas' alertCount={this.state.list.length} notificationCount={this.state.notificationCount} />
        <Text style={[styles.title, { marginVertical: 10, textAlign: 'center', color: 'slateblue' }]}>Deslize para direita ou esquerda para arquivar um alerta!</Text>
